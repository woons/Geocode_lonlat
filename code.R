...
# sample data.frame
my_address <- data.frame(address = c("제주 특별자치도 제주시 첨단로 242", "부산광역시 동래구 안락1동 921-5", "서울특별시 구로구 공원로6가길"),
                         category = c("회사", "자택", "자택"),
                         etc = c("woons", NA, NA)
)
...
# sample vector
sample_vector <- c("제주 특별자치도 제주시 첨단로 242", "부산광역시 동래구 안락1동 921-5", "서울특별시 구로구 공원로6가길")
...
#final
final <- cbind(my_address, final_lonlat)
final
